...
leftpad str n = take (n - length str) (repeat ' ') ++ str  
...
smallestDivisor n = smallestDivisor' n 2
  where smallestDivisor' n m = if mod n m == 0 then m else smallestDivisor' n (m+1)
...
biggestPrimeAtMost = (\n -> if isPrime n then n else biggestPrimeAtMost (n-1))
