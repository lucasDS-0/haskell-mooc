...
sumsHelp :: [Int] -> [Int]
sumsHelp []     = []
sumsHelp (x:xs) = (x*(x+1) `div` 2) : sumsHelp xs 
...
maybeMap f xs = maybeAux (mapAux f xs) 
                  
mapAux :: (a -> Maybe b) -> [a] -> [Maybe b]
mapAux _ []     = []
mapAux f (x:xs) = f x : mapAux f xs
                  
maybeAux :: [Maybe b] -> [b]
maybeAux []     = []
maybeAux (x:xs) = case x of
    Just a  -> a : maybeAux xs
    Nothing -> maybeAux xs
